...
class FaceGuideImage(StreamingInput, Component):
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A PIL FaceGuideImage, numpy array, path or URL for the default value that FaceGuideImage component is going to take. If callable, the function will be called whenever the app loads to set the initial value of the component.
            format: Format to save image if it does not already have a valid format (e.g. if the image is being returned to the frontend as a numpy array or PIL FaceGuideImage).  The format should be supported by the PIL library. This parameter has no effect on SVG files.
        ...
        if streaming and self.sources != ["webcam"]:
            raise ValueError(
                "FaceGuideImage streaming only available if sources is ['webcam']. Streaming not supported with multiple sources."
            )
